...
#       # For verbose output add "SILENT=FALSE" to command line(s) below
	...
	make build/onebit_comp.correct.txt   DELAY=0,0 GOLD=ignore
...
build/%_mapped.json: build/%_design_top.json
	...
	@echo "MAPPER"
	@echo; echo Making $@ because of $?
	cgra-mapper build/$*_design_top.json build/$*_mapped.json $(OUTPUT)
...
#	od -t u1 build/$*_CGRA_out.raw   | head -2
	...
	diff --side-by-side -W 12 build/$*_halide_out.od build/$*_CGRA_out.od | head -50
	@echo
